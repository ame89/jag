...
// cmd/sqlite2postgres, which needs to enumerate the whole model_node
// table (not just look up specific IDs like GetNodesByIDs).
func (m *ModelStore) AllNodes(afterID string, limit int) ([]coremodel.Node, error) {
	rows, err := m.db.Query(
		`SELECT id, kind FROM model_node WHERE id > ? ORDER BY id LIMIT ?`,
		afterID, limit,
	)
	if err != nil {
		return nil, fmt.Errorf("sqlite: paging nodes: %w", err)
	}
	defer rows.Close()
...
// cmd/sqlite2postgres. Never lets a page end mid-owner (same rationale as
// AllAttributes' owner-continuation fix above) so a caller building an
// owner->node->group map for UpsertElectricalGroups never sees a partial
// owner's rows.
func (m *ModelStore) AllElectricalGroups(afterOwnerID string, limit int) ([]coremodel.ElectricalGroupRow, error) {
	rows, err := m.db.Query(
		`SELECT node_id, owner_id, group_id FROM model_electrical_group WHERE owner_id > ? ORDER BY owner_id, node_id LIMIT ?`,
		afterOwnerID, limit,
	)
	if err != nil {
		return nil, fmt.Errorf("sqlite: paging electrical groups: %w", err)
	}
	...
	for len(result) > 0 && len(result) >= limit {
		...
		moreRows, err := m.db.Query(
			`SELECT node_id, owner_id, group_id FROM model_electrical_group WHERE owner_id = ? AND node_id > ? ORDER BY node_id LIMIT ?`,
			lastOwner, lastNode, limit,
		)
		if err != nil {
			return nil, fmt.Errorf("sqlite: paging electrical groups (owner continuation): %w", err)
		}
