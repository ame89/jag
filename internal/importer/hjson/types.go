...
// Busbar is one named busbar within a Substation/KVS file, grouping one or
// more BusbarSection-equivalent Sections (each its own Node/connects
// endpoint, no separate node name needed — see Konzept.md's worked
// example).
type Busbar struct {
	ID       string               `json:"id"`
