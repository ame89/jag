...
"HJSON Fachmodell"-Abschnitt) liest exakt dieselben acht Variablen `JAG_DB_PATH`,
`JAG_CHUNK_SIZE`, `JAG_STATION_BATCH_SIZE`, `JAG_STATION_WORKERS`, `JAG_PASS_B_WORKERS`,
`JAG_PASS_B_BATCH_SIZE`, `JAG_KEEP_STAGING` und `JAG_SKIP_VACUUM` mit identischen Defaults (nur der Default-Dateiname für
